  ...
  return (
    <div className="relative">
      <svg
        ...
        aria-hidden
      >
        {/* Connecting lines — draw on (ink traces on paper) */}
        {lines.map((l, i) => (
          <motion.line
            key={i}
            x1={l.from[0]}
            ...
            x2={l.to[0]}
            y2={l.to[1]}
            style={{ stroke: "var(--color-ink-light)" }}
            strokeWidth="1.5"
            strokeDasharray="8 6"
            initial={reduced ? {} : { pathLength: 0, opacity: 0 }}
            animate={inView ? { pathLength: 1, opacity: 1 } : {}}
            transition={{
              pathLength: { duration: 1.2, delay: l.delay, ease: [0.2, 0.8, 0.2, 1] },
              opacity: { duration: 0.4, delay: l.delay },
            }}
          />
        ))}

        {/* Outer nodes — opaque white panels, square, rule-soft border */}
        {nodes.map((n) => (
          <motion.g
            key={n.id}
          ...
          >
            ...
            <text
              ...
              y={n.y + 56}
              textAnchor="middle"
              style={{ fill: "var(--color-ink-light)" }}
              fontSize="11"
              fontFamily="var(--font-mono)"
            >
              {n.sub}
        ...
        {/* Center node: AI Agent — the active path, accented in red */}
        <motion.g
        ...
        >
          ...
          <motion.rect
            ...
            height="96"
            fill="none"
            style={{ stroke: "var(--color-red-mid)" }}
            strokeWidth="1"
            animate={reduced ? {} : { strokeOpacity: [0.4, 1, 0.4] }}
          ...
          <rect
            ...
            width="140"
            height="80"
            style={{ fill: "var(--color-paper-raised)" }}
          />
          <motion.rect
            x="170"
            y="190"
            ...
            strokeWidth="1.5"
            animate={reduced ? {} : { strokeOpacity: [0.5, 1, 0.5] }}
        ...
        {/* Traveling dots — red pulses along the active traces */}
