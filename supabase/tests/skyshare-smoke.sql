...
select pg_temp.check_ok(exists(select 1 from organizations where id='85ded2c8-d4b0-4109-b3c0-ef8c15ab4922' and slug='acme-staging'), 'staging fixture tenant');
select pg_temp.check_ok(exists(select 1 from auth.users where id='cc1b47e3-4cb7-47f7-b874-677d9c37a532' and email='brayden@supraintegration.ai' and email_confirmed_at is not null and last_sign_in_at is not null), 'confirmed staging user has signed in');
select pg_temp.check_ok((select count(*)=1 from organization_memberships where user_id='cc1b47e3-4cb7-47f7-b874-677d9c37a532') and exists(select 1 from organization_memberships where user_id='cc1b47e3-4cb7-47f7-b874-677d9c37a532' and organization_id='85ded2c8-d4b0-4109-b3c0-ef8c15ab4922' and role='client_member' and status='active'), 'client has only expected membership');
-- No hunt, user, membership or configuration creation. Reuse the SEC hunt.
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"5757107e-69b1-48e5-adf3-07f3de2211e2","role":"authenticated"}',true);
...
insert into lead_candidates(id,organization_id,supra_lead_id,source_hunt_key,source_hunt_label,person_name,trigger_summary,why_found,system_recommendation,publication_state,published_at,published_by) values('eeeeeeee-0000-4000-8000-000000000004','2540997c-7bbb-4430-a431-729fc258f431','SMOKE-OTHER-TENANT','sec-insider-sale-5m','SEC smoke fixture','Other tenant sentinel','Fixture','Isolation check','whale','published',now(),'5757107e-69b1-48e5-adf3-07f3de2211e2');
select set_config('request.jwt.claims','{"sub":"cc1b47e3-4cb7-47f7-b874-677d9c37a532","role":"authenticated"}',true);
...
begin
insert into lead_feedback(organization_id,candidate_id,user_id,human_decision) values(case when target='eeeeeeee-0000-4000-8000-000000000004' then '2540997c-7bbb-4430-a431-729fc258f431'::uuid else '85ded2c8-d4b0-4109-b3c0-ef8c15ab4922'::uuid end,target,'cc1b47e3-4cb7-47f7-b874-677d9c37a532','approve');
...
select set_config('request.jwt.claims','{"sub":"5757107e-69b1-48e5-adf3-07f3de2211e2","role":"authenticated"}',true);
update lead_candidates set publication_state='published',published_at=now(),published_by='5757107e-69b1-48e5-adf3-07f3de2211e2' where id='eeeeeeee-0000-4000-8000-000000000003';
select set_config('request.jwt.claims','{"sub":"cc1b47e3-4cb7-47f7-b874-677d9c37a532","role":"authenticated"}',true);
...
insert into lead_feedback(organization_id,candidate_id,user_id,human_decision) values('85ded2c8-d4b0-4109-b3c0-ef8c15ab4922','eeeeeeee-0000-4000-8000-000000000003','cc1b47e3-4cb7-47f7-b874-677d9c37a532','approve');
...
rollback;
select 'PASS: staging database contract; all fixture changes rolled back; browser interactions not asserted' as result;
