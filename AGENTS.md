...
- The active visual direction is a light, spatial operating system on cream paper. Full-page black sections are retired.
- Palette: paper `#F6F5F1`, paper-soft `#ECEAE3`, ink `#0E0E10`, ink-soft `#2A2A2E`, ink-mid `#5A5A60`, rule `#C2BFB6`. Red `#E6172A` is the ONLY accent.
- No gold, purple, blue, cyan, or multicolor accents. Stars and environmental highlights may only be black or red.
- No gradients on text or buttons. Environmental gradients are allowed only to create restrained spatial depth. Controlled red glow is allowed on stars, orbit lines, and the central system core—not on ordinary cards, copy, or buttons.
- A limited black/red flickering star field is part of the design. Keep it sparse and render it with CSS backgrounds or a small fixed set of elements; never create hundreds of DOM particles.
- The primary page canvas stays cream. Cards, contained panels, buttons, and system cores may use ink with cream text for deliberate contrast; do not use white cards on cream.
